...
class VehicleVanPermissionScreen extends StatefulWidget {
  const VehicleVanPermissionScreen({Key? key}) : super(key: key);

  @override
  State<VehicleVanPermissionScreen> createState() => _VehicleVanPermissionScreenState();
}

class _VehicleVanPermissionScreenState extends State<VehicleVanPermissionScreen> {

  Map<String, dynamic> _user = {};
  final String _codeAccess = "04";
  ...
  void getPermissionToday() async {
    ...
    await AuthRepo.instance.getSession("vehicle_van_permission").then((value){
      if(value != null){
  ...
  void permissionRequest({String userId = "", bool keluar = false, bool masuk = false, String tujuan = "", String jenisKendaraan = "", String qrcode = ""}) async {
    ...
    await VehicleVanPermissionRepo.instance.store(dataBatch).then((value) {
      getPermissionToday();
    });
  }
